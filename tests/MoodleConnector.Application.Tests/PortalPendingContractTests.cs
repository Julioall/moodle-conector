...
public sealed class PortalPendingContractTests
{
    [Fact]
    public void Pending_item_preserves_compound_identity_and_objective_reasons()
    {
        var item = new PortalPendingDto(
    ...
    public void Contract_mapper_creates_access_and_submission_items_without_mutation_capabilities()
    {
        ...
        var items = PortalPendingContractMapper.Build(
            "senai-goias",
            "course-1",
            new[]
            {
                new PortalPendingSourceRow("student-1", "Ana", null, "activity-1", "Entrega", "pending_submission", DateTimeOffset.UtcNow.AddDays(2), false, false)
            },
            new[]
            {
                new PortalPendingAccessRow("student-2", "Bruno", DateTimeOffset.UtcNow.AddDays(-45))
            },
            generatedAt);
