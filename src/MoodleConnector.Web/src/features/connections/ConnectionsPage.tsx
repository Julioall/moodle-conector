  ...
  return (
    <Card className="connection-card">
      ...
      <CardContent>
        ...
        <p className="connection-validation">{connection.lastValidatedAt ? `Último teste em ${formatFreshness(connection.lastValidatedAt).replace('Atualizado em ', '')}` : 'Ainda não testada nesta versão do portal'}</p>
...
export function ConnectionsPage() {
  ...
  const updateMutation = useMutation({ mutationFn: ({ id, input }: { id: string; input: Parameters<typeof connectionsGateway.update>[1] }) => connectionsGateway.update(id, input), onSuccess: () => { setSuccess('Conexão atualizada com sucesso.'); setEditingConnection(undefined); setAlias(''); setBaseUrl(''); setUsername(''); setPassword(''); void queryClient.invalidateQueries({ queryKey: ['portal', 'connections'] }); } });
  const validate = async (connectionRef: string) => {
    setValidatingRef(connectionRef);
    setValidationError('');
    try { await connectionsGateway.validate(connectionRef); await queryClient.invalidateQueries({ queryKey: ['portal', 'connections'] }); }
