...
export const createFollowupGateway = (client: PortalClient = createPortalClient()) => ({ list: () => client.get<FollowupList>('/api/portal/followups'), create: async (input: FollowupInput) => { await client.get('/api/portal/csrf'); return client.request('/api/portal/followups', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) }); } });
