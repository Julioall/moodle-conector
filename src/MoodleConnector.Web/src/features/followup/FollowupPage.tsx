...
export function FollowupPage() {
  ...
  const query = useQuery({ queryKey: ['app', 'followups'], queryFn: followupGateway.list, staleTime: 30_000 });
  const [studentRef, setStudentRef] = useState('');
  const [courseRef, setCourseRef] = useState('');
  const [notes, setNotes] = useState('');
  const [kind, setKind] = useState('acompanhamento');
  const create = useMutation({ mutationFn: (input: FollowupInput) => followupGateway.create(input), onSuccess: () => { setStudentRef(''); setCourseRef(''); setNotes(''); setKind('acompanhamento'); void client.invalidateQueries({ queryKey: ['app', 'followups'] }); } });
  const records = query.data?.data ?? [];

  return <main className="content-frame" aria-labelledby="followup-title"><header className="page-heading"><div><p className="eyebrow">OPERACIONAL</p><h1 id="followup-title">Follow-up</h1><p>Registre contatos e acompanhamentos humanos com rastreabilidade.</p></div><div className="flex items-center gap-3">{query.data && <span className="freshness">Atualizado em {new Date(query.data.meta.generatedAt).toLocaleString('pt-BR')}</span>}<Button type="button" variant="outline" onClick={() => void query.refetch()} disabled={query.isFetching}><RefreshCw className={query.isFetching ? 'animate-spin' : ''} />Atualizar</Button></div></header>
    <div className="grid gap-6 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]"><Card><CardHeader><CardTitle className="flex items-center gap-2 text-lg"><Plus className="h-5 w-5 text-primary" />Registrar acompanhamento</CardTitle><CardDescription>Documente uma ação humana vinculada ao aluno e, quando possível, ao curso.</CardDescription></CardHeader><CardContent><form className="grid gap-4" onSubmit={(event) => { event.preventDefault(); if (studentRef.trim() && notes.trim()) create.mutate({ studentRef: studentRef.trim(), courseRef: courseRef.trim() || undefined, notes: notes.trim(), kind }); }}><label className="grid gap-1.5 text-sm font-medium">Referência do aluno<Input value={studentRef} onChange={(event) => setStudentRef(event.target.value)} placeholder="connectionRef:studentId" required /></label><label className="grid gap-1.5 text-sm font-medium">Referência do curso <span className="font-normal text-muted-foreground">opcional</span><Input value={courseRef} onChange={(event) => setCourseRef(event.target.value)} placeholder="connectionRef:courseId" /></label><label className="grid gap-1.5 text-sm font-medium">Tipo de acompanhamento<select className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" value={kind} onChange={(event) => setKind(event.target.value)}>{Object.entries(kindLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="grid gap-1.5 text-sm font-medium">Notas do acompanhamento<Textarea className="min-h-36" value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Descreva o contato, a orientação ou o próximo passo." required /></label><Button type="submit" disabled={create.isPending || !studentRef.trim() || !notes.trim()}>{create.isPending ? 'Registrando…' : 'Registrar acompanhamento'}</Button></form>{create.isError && <p className="mt-4 text-sm text-destructive" role="alert">Não foi possível registrar o acompanhamento.</p>}{create.isSuccess && <p className="mt-4 text-sm text-status-success" role="status">Acompanhamento registrado com sucesso.</p>}</CardContent></Card>
