...
export type CourseScope = 'active' | 'all';
...
export const createCoursesGateway = (client = createPortalClient()) => ({
  list: (connectionRef?: string, page = 1, pageSize = 20, scope: CourseScope = 'active') => client.get<ListResponse<Course>>(`/api/portal/courses?${new URLSearchParams({ ...(connectionRef ? { connectionRef } : {}), scope, page: String(page), pageSize: String(pageSize) })}`),
