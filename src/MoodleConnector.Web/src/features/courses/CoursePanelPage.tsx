...
export function CoursePanelPage() {
  ...
  const studentCount = students.data?.meta.total ?? students.data?.data.length ?? 0;
  ...
  const studentTotalPages = Math.max(1, Math.ceil(studentCount / 25));
      ...
      {data && (
        <>
          ...
          <div className="grid gap-4 md:grid-cols-4">
            <Card><CardContent className="p-4"><div className="flex items-center gap-3"><div className="rounded-lg bg-primary/10 p-2"><Users className="h-5 w-5 text-primary" /></div><div><p className="text-2xl font-bold">{studentCount}</p><p className="text-xs text-muted-foreground">Alunos matriculados</p></div></div></CardContent></Card>
            <Card><CardContent className="p-4"><div className="flex items-center gap-3"><div className="rounded-lg bg-risk-risco/10 p-2"><AlertTriangle className="h-5 w-5 text-risk-risco" /></div><div><p className="text-2xl font-bold">{students.isPending ? '—' : atRiskCount}</p><p className="text-xs text-muted-foreground">Alunos em risco na página</p></div></div></CardContent></Card>
          ...
          <Tabs defaultValue="overview" className="space-y-4">
            <TabsList aria-label="Seções do curso">
              ...
              <TabsTrigger value="students">Alunos ({studentCount})</TabsTrigger>
