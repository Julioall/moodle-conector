...
export function CoursePanelPage() {
  ...
  const course = useQuery({
    queryKey: ['portal', 'course', connectionRef, courseId],
    queryFn: () => coursesGateway.get(connectionRef, courseId),
    enabled,
  });
  const activities = useQuery({
    queryKey: ['portal', 'course-activities', connectionRef, courseId],
    queryFn: () => coursesGateway.activities(connectionRef, courseId),
    enabled,
  });
  const students = useQuery({
    queryKey: ['portal', 'course-students', connectionRef, courseId],
      ...
      {data && (
        <>
          <header className="flex flex-col gap-4 rounded-lg border bg-card p-6 shadow-sm md:flex-row md:items-start md:justify-between">
            <div>
              <p className="mb-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">Curso · {connectionRef}</p>
          ...
          <Tabs defaultValue="overview" className="space-y-4">
            ...
            <TabsContent value="overview" className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-3">
                <Card><CardHeader className="pb-2"><CardDescription>Progresso</CardDescription><CardTitle className="text-2xl">{data.progress != null ? `${Math.round(data.progress)}%` : '—'}</CardTitle></CardHeader><CardContent className="text-xs text-muted-foreground">Acompanhamento retornado pelo Moodle</CardContent></Card>
                <Card><CardHeader className="pb-2"><CardDescription>Início</CardDescription><CardTitle className="text-base">{formatDate(data.startDate)}</CardTitle></CardHeader><CardContent><Calendar className="h-4 w-4 text-muted-foreground" /></CardContent></Card>
              ...
              <Card><CardHeader><CardTitle>Contexto do curso</CardTitle><CardDescription>Dados consultados na conexão Moodle selecionada.</CardDescription></CardHeader><CardContent className="grid gap-3 text-sm sm:grid-cols-2"><div><span className="text-muted-foreground">Identidade composta</span><p className="font-mono text-xs">{connectionRef}:{courseId}</p></div><div><span className="text-muted-foreground">Último acesso</span><p>{formatDate(data.lastAccessAt)}</p></div></CardContent></Card>
            </TabsContent>

            <TabsContent value="students">
              <Card><CardHeader><CardTitle className="flex items-center gap-2"><Users className="h-5 w-5 text-primary" /> Alunos do curso</CardTitle><CardDescription>Participantes limitados ao curso e à conexão atuais.</CardDescription></CardHeader><CardContent>
