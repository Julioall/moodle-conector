...
import { AlertTriangle, ArrowLeft, CheckCircle2, ClipboardList, ExternalLink, FileCheck2, MessageCircle, RefreshCw, Search, Users } from 'lucide-react';
...
function followupStudentLabel(studentName: string | undefined, studentRef: string) {
  if (studentName?.trim()) return studentName;
  const [, studentId] = studentRef.split(':', 2);
  return studentId ? `Aluno ${studentId}` : 'Aluno não identificado';
}
...
export function CoursePanelPage() {
  ...
  const activityWarnings = activities.data?.meta.warnings ?? [];
  const studentWarnings = students.data?.meta.warnings ?? [];
  const dashboardWarnings = dashboard.data?.data.warnings ?? [];
  const prioritiesPreparing = dashboardWarnings.some((warning) =>
    warning.toLocaleLowerCase('pt-BR').includes('prioridades do curso estão sendo preparadas'));
  const priorityActionCount = Math.max(
    dashboard.data?.data.priorities.length ?? 0,
    (summary?.studentsAtRisk ?? 0) + (summary?.pendingSubmissionAssignments ?? 0) + (summary?.pendingCorrectionAssignments ?? 0),
  );
  ...
  const refreshCourseData = async () => {
    ...
    try {
      const refreshTasks: Array<Promise<unknown>> = [
        ...
        queryClient.fetchQuery({
          queryKey: ['app', 'course-dashboard', connectionRef, courseId],
          queryFn: () => dashboardGateway.get(connectionRef, courseId, false, 'current', true),
          staleTime: 0,
        }),
      ];
      ...
      if (activeTab === 'overview' || activeTab === 'activities') {
        refreshTasks.push(queryClient.fetchQuery({
          queryKey: ['app', 'course-activities', connectionRef, courseId, activitiesPage],
          queryFn: () => coursesGateway.activities(connectionRef, courseId, activitiesPage, 20, false, true),
          staleTime: 0,
        }));
      }

      if (activeTab === 'students' || followupOpen) {
        refreshTasks.push(queryClient.fetchQuery({
          queryKey: ['app', 'course-students', connectionRef, courseId, studentsPage],
          queryFn: () => studentsGateway.byCourse(connectionRef, courseId, studentsPage, 25, false, true),
          staleTime: 0,
        }));
      }
      ...
      if (activeTab === 'overview') {
        refreshTasks.push(queryClient.fetchQuery({
          queryKey: ['app', 'followups', connectionRef, courseId],
          queryFn: () => followupGateway.list({ connectionRef, courseId }),
          staleTime: 0,
        }));
      }
    ...
    <main className="space-y-6 animate-fade-in" aria-labelledby="course-title">
      ...
      {data && <>
        <header className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3 md:justify-end"><div className="hidden text-right text-xs text-muted-foreground sm:block"><span>Última consulta:</span><span className="ml-1 font-medium">{formatDateTime(course.data?.meta.generatedAt)}</span></div><Button variant="outline" size="sm" onClick={() => void refreshCourseData()} disabled={isRefreshing}><RefreshCw className={`h-4 w-4 ${isRefreshing ? 'animate-spin' : ''}`} />Atualizar</Button><Badge variant="outline">Somente leitura</Badge>{data.viewUrl && <Button variant="outline" size="sm" asChild><a href={data.viewUrl} target="_blank" rel="noreferrer">Abrir no Moodle <ExternalLink className="h-4 w-4" /></a></Button>}</div>
        </header>
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-4"><TabsList className="h-auto w-full flex-wrap justify-start gap-1" aria-label="Seções do curso"><TabsTrigger value="overview">Visão geral</TabsTrigger><TabsTrigger value="students">Alunos{studentCount == null ? '' : ` (${studentCount})`}</TabsTrigger><TabsTrigger value="activities">Atividades{activityCount ? ` (${activityCount})` : ''}</TabsTrigger></TabsList>
          <TabsContent value="overview" className="space-y-4">
            <Card><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle className="text-base">Prioridades</CardTitle><CardDescription>Sinais que pedem uma decisão do tutor.</CardDescription></div>{summary && <Badge variant={priorityActionCount > 0 ? 'destructive' : 'outline'}>{priorityActionCount} {priorityActionCount === 1 ? 'ação sugerida' : 'ações sugeridas'}</Badge>}</div></CardHeader><CardContent>{dashboard.isPending && <div className="space-y-3"><Skeleton className="h-10 w-full" /><Skeleton className="h-10 w-full" /></div>}{dashboard.isError && <p role="alert" className="text-sm text-destructive">Não foi possível carregar o estado da turma.</p>}{dashboard.isSuccess && dashboardWarnings.length > 0 && <div className="mb-4 rounded-lg border border-amber-200/70 bg-amber-50/60 p-3 text-xs text-amber-900">{dashboardWarnings.map((warning) => <p key={warning}>{warning}</p>)}</div>}{dashboard.isSuccess && prioritiesPreparing && dashboard.data.data.priorities.length === 0 && <div className="flex items-center gap-3 rounded-lg border border-dashed p-4 text-sm text-muted-foreground"><ClipboardList className="h-5 w-5 text-amber-600" />As prioridades serão exibidas assim que a atualização terminar.</div>}{dashboard.isSuccess && !prioritiesPreparing && summary && summary.studentsAtRisk === 0 && (summary.pendingSubmissionAssignments ?? 0) === 0 && (summary.pendingCorrectionAssignments ?? 0) === 0 && <div className="flex items-center gap-3 rounded-lg border border-dashed p-4 text-sm text-muted-foreground"><CheckCircle2 className="h-5 w-5 text-green-600" />Nenhuma prioridade operacional identificada.</div>}{dashboard.isSuccess && dashboard.data.data.priorities.length > 0 && <div className="divide-y">{dashboard.data.data.priorities.slice(0, 8).map((item) => <div key={item.key} className="flex flex-wrap items-center justify-between gap-3 py-3 first:pt-0"><div className="flex items-start gap-3"><div className="mt-0.5 rounded-full bg-muted p-1.5">{item.level === 'risk' ? <AlertTriangle className="h-4 w-4 text-risk-risco" /> : <ClipboardList className="h-4 w-4 text-amber-600" />}</div><div><p className="font-medium">{item.title}</p><p className="text-sm text-muted-foreground">{item.detail}</p></div></div>{item.studentId && <Button variant="outline" size="sm" asChild><Link to={`/cursos/${encodeURIComponent(connectionRef)}/${encodeURIComponent(courseId)}/alunos/${encodeURIComponent(item.studentId)}`}>Ver aluno</Link></Button>}</div>)}</div>}</CardContent></Card>
            <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle className="text-base">Histórico de acompanhamento</CardTitle><CardDescription>Intervenções realizadas por tutores, monitores e analistas nesta turma.</CardDescription></div>{followups.data && <Badge variant="outline">{followups.data.data.length} {followups.data.data.length === 1 ? 'registro' : 'registros'}</Badge>}</div></CardHeader><CardContent>{followups.isPending && <div className="space-y-3"><Skeleton className="h-14 w-full" /><Skeleton className="h-14 w-full" /></div>}{followups.isError && <p role="alert" className="text-sm text-destructive">Não foi possível carregar o histórico de acompanhamento.</p>}{followups.isSuccess && followups.data.data.length === 0 && <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">Nenhuma intervenção registrada nesta turma.</div>}{followups.isSuccess && followups.data.data.length > 0 && <div className="divide-y">{followups.data.data.slice(0, 8).map((record) => <article key={record.id} className="py-4 first:pt-0"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><p className="font-medium">{followupStudentLabel(record.studentName, record.studentRef)}</p><div className="mt-1 flex flex-wrap items-center gap-2"> <Badge variant="secondary">{actionLabels[record.action ?? ''] ?? 'Acompanhamento'}</Badge>{record.status && <Badge variant="outline">{statusLabels[record.status] ?? record.status}</Badge>}</div><p className="mt-2 text-xs text-muted-foreground">{record.reason ? reasonLabels[record.reason] ?? record.reason : 'Acompanhamento'} · por {record.actorName ?? 'Usuário'} · {formatDateTime(record.occurredAt)}</p></div></div><p className="mt-2 whitespace-pre-wrap text-sm text-muted-foreground">{record.notes}</p></article>)}</div>}{followups.isSuccess && followups.data.meta.hasMore && <p className="mt-3 text-xs text-muted-foreground">Mostrando os 8 registros mais recentes.</p>}</CardContent></Card>
          </TabsContent>
          <TabsContent value="students"><Card><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle className="text-base">Alunos do curso</CardTitle><CardDescription>Lista operacional com risco e último acesso.</CardDescription></div><Button size="sm" onClick={() => setFollowupOpen(true)}><MessageCircle className="h-4 w-4" />Registrar acompanhamento</Button></div></CardHeader><CardContent>
            ...
            {students.isPending && <div className="space-y-3"><Skeleton className="h-10 w-full" /><Skeleton className="h-10 w-full" /></div>}{students.isError && <p role="alert" className="text-destructive">Não foi possível carregar os alunos.</p>}{students.isSuccess && studentWarnings.length > 0 && <div className="mb-4 rounded-lg border border-amber-200/70 bg-amber-50/60 p-3 text-xs text-amber-900">{studentWarnings.map((warning) => <p key={warning}>{warning}</p>)}</div>}{students.isSuccess && visibleStudents.length === 0 && <p className="py-8 text-center text-sm text-muted-foreground">{studentWarnings.some((warning) => warning.includes('sendo preparada')) ? 'A lista será exibida assim que a atualização terminar.' : 'Nenhum aluno corresponde aos filtros atuais.'}</p>}{students.isSuccess && visibleStudents.length > 0 && <div className="divide-y">{visibleStudents.map((student: Student) => <div key={`${student.connectionRef}:${student.studentId}`} className="grid gap-3 py-4 first:pt-0 md:grid-cols-[minmax(0,1fr)_150px_140px_auto] md:items-center"><div><p className="font-medium">{student.name}</p><p className="text-xs text-muted-foreground">{student.email ?? 'E-mail não informado'}</p></div><div><p className="text-xs text-muted-foreground">Situação</p><Badge variant={isAtRisk(student.risk) ? 'destructive' : 'secondary'}>{riskLabel(student.risk)}</Badge></div><div><p className="text-xs text-muted-foreground">Último acesso</p><p className="text-sm">{student.lastCourseAccessAt ? formatDateTime(student.lastCourseAccessAt) : 'Nunca acessou'}</p></div><Button variant="outline" size="sm" asChild><Link to={`/cursos/${encodeURIComponent(connectionRef)}/${encodeURIComponent(courseId)}/alunos/${encodeURIComponent(student.studentId)}`}>Abrir perfil</Link></Button></div>)}</div>}{studentTotalPages > 1 && <div className="mt-4 flex items-center justify-between gap-3 border-t pt-4 text-xs text-muted-foreground"><span>Página {studentsPage} de {studentTotalPages}</span><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => setStudentsPage((page) => Math.max(1, page - 1))} disabled={studentsPage <= 1}>Anterior</Button><Button variant="outline" size="sm" onClick={() => setStudentsPage((page) => Math.min(studentTotalPages, page + 1))} disabled={studentsPage >= studentTotalPages}>Próxima</Button></div></div>}
          </CardContent></Card></TabsContent>
          <TabsContent value="activities" className="space-y-6"><Card><CardHeader><div><CardTitle className="text-base">Atividades</CardTitle><CardDescription>Lista das atividades do curso para consulta rápida e abertura direta no Moodle.</CardDescription></div></CardHeader><CardContent>{activities.isPending && <div className="space-y-3"><Skeleton className="h-12 w-full" /><Skeleton className="h-12 w-full" /></div>}{activities.isError && <p role="alert" className="text-destructive">Não foi possível carregar as atividades.</p>}{activities.isSuccess && activityWarnings.length > 0 && <div className="mb-4 rounded-lg border border-amber-200/70 bg-amber-50/60 p-3 text-xs text-amber-900">{activityWarnings.map((warning) => <p key={warning}>{warning}</p>)}</div>}{activities.isSuccess && activities.data.data.length === 0 && <div className="rounded-lg border border-dashed p-6 text-center"><CheckCircle2 className="mx-auto h-6 w-6 text-green-600" /><p className="mt-2 text-sm font-medium">{activityWarnings.some((warning) => warning.includes('sendo preparada')) ? 'A lista será exibida assim que a atualização terminar.' : 'Nenhuma atividade encontrada nesta página.'}</p></div>}{activities.isSuccess && activities.data.data.length > 0 && <div className="divide-y">{activities.data.data.map((activity) => <article className="flex flex-wrap items-center justify-between gap-3 py-4 first:pt-0" key={`${activity.connectionRef}:${activity.courseId}:${activity.activityId}`}><div className="min-w-0"><p className="font-medium">{activity.name}</p><p className="text-xs text-muted-foreground">{activity.activityType}{activity.dueAt ? ` · prazo ${formatDate(activity.dueAt)}` : ' · sem prazo'}</p></div>{activity.url && <a className="flex shrink-0 items-center gap-1 text-sm text-primary hover:underline" href={activity.url} target="_blank" rel="noreferrer">Abrir no Moodle <ExternalLink className="h-3.5 w-3.5" /></a>}</article>)}</div>}{activityTotalPages > 1 && <div className="mt-4 flex items-center justify-between gap-3 border-t pt-4 text-xs text-muted-foreground"><span>Página {activitiesPage} de {activityTotalPages}</span><div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => setActivitiesPage((page) => Math.max(1, page - 1))} disabled={activitiesPage <= 1}>Anterior</Button><Button variant="outline" size="sm" onClick={() => setActivitiesPage((page) => Math.min(activityTotalPages, page + 1))} disabled={activitiesPage >= activityTotalPages}>Próxima</Button></div></div>}</CardContent></Card><PendingCorrectionsPage embedded courseContext={{ connectionRef, courseId, courseName: data.displayName ?? data.fullName }} /></TabsContent>
        </Tabs>
