      ...
      {data && <>
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-4"><TabsList className="h-auto w-full flex-wrap justify-start gap-1" aria-label="Seções do curso"><TabsTrigger value="overview">Visão geral</TabsTrigger><TabsTrigger value="students">Alunos{studentCount == null ? '' : ` (${studentCount})`}</TabsTrigger><TabsTrigger value="activities">Atividades{activityCount ? ` (${activityCount})` : ''}</TabsTrigger></TabsList>
          <TabsContent value="overview" className="space-y-4">
            ...
            <Card><CardHeader><div className="flex flex-wrap items-center justify-between gap-3"><div><CardTitle className="text-base">Atividades que precisam de ação</CardTitle><CardDescription>Entregas pendentes e itens aguardando correção.</CardDescription></div><Button variant="outline" size="sm" onClick={() => handleTabChange('activities')}>Ver atividades</Button></div></CardHeader><CardContent><div className="grid gap-3 sm:grid-cols-2"><div className="rounded-lg border p-4"><p className="text-2xl font-bold">{summary?.pendingSubmissionAssignments ?? '—'}</p><p className="text-sm text-muted-foreground">Entregas ainda não realizadas</p></div><div className="rounded-lg border p-4"><p className="text-2xl font-bold">{summary?.pendingCorrectionAssignments ?? '—'}</p><p className="text-sm text-muted-foreground">Submissões para correção no MCP + ChatGPT</p></div></div></CardContent></Card>
          </TabsContent>
