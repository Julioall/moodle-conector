...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  if (session.error && !(session.error instanceof AppHttpError && session.error.status === 401)) return <div className="foundation-card"><h1>Sessão indisponível</h1><p>Não foi possível validar sua sessão.</p><button className="ui-button ui-button-outline" onClick={() => void session.refetch()}>Tentar novamente</button></div>;
