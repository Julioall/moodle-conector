...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  if (!session.data?.data.authenticated) return <div className="foundation-card"><h1>Login necessário</h1><a href="/auth/login?returnUrl=/">Entrar no Moodle Connector</a></div>;
  return <>{children}</>;
}
