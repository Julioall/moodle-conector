...
export const createAgendaGateway = (client: AppClient = createAppClient()) => ({ list: () => client.get<AgendaResponse>('/api/agenda'), create: async (input: AgendaInput) => { await client.get('/api/csrf'); return client.request('/api/agenda', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) }); }, update: async (id: string, input: AgendaInput) => { await client.get('/api/csrf'); return client.request(`/api/agenda/${encodeURIComponent(id)}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(input) }); }, remove: async (id: string) => { await client.get('/api/csrf'); return client.request(`/api/agenda/${encodeURIComponent(id)}`, { method: 'DELETE' }); } });
