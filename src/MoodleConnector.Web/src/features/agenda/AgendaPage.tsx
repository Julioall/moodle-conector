...
import { AlertCircle, CalendarDays, CalendarRange, CheckSquare, ChevronLeft, ChevronRight, Clock3, Download, List, Plus, Upload } from 'lucide-react';
...
function AgendaEventCard({ event, onOpen }: { event: AgendaEvent; onOpen: (event: AgendaEvent) => void }) {
  return (
    <article className="group rounded-lg border bg-card p-4 transition-colors hover:border-primary/30 hover:bg-muted/10">
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-3 flex justify-end gap-1 border-t pt-3"><Button type="button" variant="ghost" size="sm" className="text-primary" onClick={() => onOpen(event)}>Abrir detalhes</Button></div>
        </div>
      </div>
    </article>
  );
...
function MonthCalendar({ month, items, onCreateOnDate, onSetMonth, onSetYear, onNavigate, onToday, onOpenTask, onOpenEvent }: { month: Date; items: AgendaItem[]; onCreateOnDate: (date: Date) => void; onSetMonth: (month: number) => void; onSetYear: (year: number) => void; onNavigate: (offset: number) => void; onToday: () => void; onOpenTask: (task: Task) => void; onOpenEvent: (event: AgendaEvent) => void }) {
...
export function AgendaPage() {
  ...
  const importIcs = useMutation({
    ...
    onSuccess: ({ data }) => {
      const summaries = [
        data.imported > 0 ? `${data.imported} ${data.imported === 1 ? 'novo item' : 'novos itens'}` : null,
        data.updated > 0 ? `${data.updated} ${data.updated === 1 ? 'item atualizado' : 'itens atualizados'}` : null,
        data.skipped > 0 ? `${data.skipped} ${data.skipped === 1 ? 'item ignorado' : 'itens ignorados'}` : null,
      ].filter(Boolean).join(', ');
  ...
  const openEdit = (event: AgendaEvent) => { setDetailEvent(null); setEditingEvent(event); setTitle(event.title); setDescription(event.description ?? ''); setStartAt(toDateTimeLocal(event.startAt)); setEndAt(event.endAt ? toDateTimeLocal(event.endAt) : ''); setType(event.type); setFormOpen(true); };
  ...
  return (
    <main className="space-y-6 animate-fade-in" aria-labelledby="agenda-title">
      ...
      {!isLoading && !isError && viewMode === 'list' && agendaItems.length > 0 && <div className="space-y-6">{Object.entries(groupedItems).map(([group, groupItems]) => <div key={group}><h2 className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">{formatGroupLabel(group)}</h2><div className="space-y-2">{groupItems.map((item) => item.kind === 'event' ? <AgendaEventCard key={`event-${item.id}`} event={item.event} onOpen={setDetailEvent} /> : <AgendaTaskCard key={`task-${item.id}`} task={item.task} onClick={setDetailTask} />)}</div></div>)}</div>}
      <TaskDetailDrawer task={detailTask} onClose={() => setDetailTask(null)} onStatusChange={(id, status) => updateTask.mutate({ id, status })} />
      <AgendaEventDetailDrawer event={detailEvent} onClose={() => setDetailEvent(null)} onEdit={openEdit} onDelete={(id) => { setDetailEvent(null); setDeleteId(id); }} />
