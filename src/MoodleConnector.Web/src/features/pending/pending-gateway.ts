...
export type PendingType = 'no_recent_access' | 'pending_submission' | 'awaiting_grading' | 'low_grade' | 'upcoming_deadline';
...
export type PendingFilters = { connectionRef?: string; courseId?: string; studentId?: string; type?: PendingType; level?: PendingLevel; periodDays?: number; page?: number; pageSize?: number };
export const createPendingGateway = (client: AppClient = createAppClient()) => ({ list: (filters: PendingFilters = {}) => { const { page = 1, pageSize = 20, ...rest } = filters; const query = new URLSearchParams({ page: String(page), pageSize: String(pageSize) }); Object.entries(rest).forEach(([key, value]) => { if (value !== undefined && value !== '') query.set(key, String(value)); }); return client.get<PendingListResponse>(`/api/pending?${query.toString()}`); } });
