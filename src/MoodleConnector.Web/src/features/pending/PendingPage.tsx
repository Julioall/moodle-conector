...
const labels: Record<PendingType, string> = {
  ...
  pending_submission: 'Atividade nÃ£o entregue',
  awaiting_grading: 'Aguardando correÃ§Ã£o',
  low_grade: 'Nota baixa',
  upcoming_deadline: 'Prazo prÃ³ximo',
};
const levelLabels: Record<PendingLevel, string> = { normal: 'Normal', attention: 'AtenÃ§Ã£o', risk: 'Risco', critical: 'CrÃ­tico' };
  ...
  return (
    <main className="content-frame space-y-6">
      <header className="page-heading">
        <div><p className="eyebrow">OPERACIONAL</p><h1>PendÃªncias</h1><p>Itens determinÃ­sticos para acompanhamento manual, sempre em modo somente leitura.</p></div>
        {query.data && <span className="freshness">Atualizado em {new Date(query.data.meta.generatedAt).toLocaleString('pt-BR')}</span>}
      </header>

      <Card>
        <CardHeader className="pb-4"><CardTitle className="flex items-center gap-2 text-lg"><BookOpen className="h-5 w-5 text-primary" /> Escopo e filtros</CardTitle><CardDescription>A consulta Ã© feita em um Ãºnico curso Moodle por vez.</CardDescription></CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-2 lg:grid-cols-4">
          <label className="grid gap-1.5 text-sm font-medium md:col-span-2" htmlFor="pending-course">Curso<select id="pending-course" aria-label="Curso" className="flex h-10 rounded-md border border-input bg-background px-3 py-2 text-sm" value={courseId ?? ''} onChange={(event) => update('courseId', event.target.value)} disabled={!connectionRef || courses.isPending}><option value="">{courses.isPending ? 'Carregando cursosâ€¦' : 'Selecione um curso'}</option>{courses.data?.data.map((course) => <option key={course.courseId} value={course.courseId}>{course.displayName ?? course.fullName} ({course.courseId})</option>)}</select></label>
          ...
          <label className="grid gap-1.5 text-sm font-medium" htmlFor="pending-level">NÃ­vel<select id="pending-level" aria-label="NÃ­vel" className="flex h-10 rounded-md border border-input bg-background px-3 py-2 text-sm" value={level ?? ''} onChange={(event) => update('level', event.target.value)}><option value="">Todos</option>{Object.entries(levelLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
          <label className="grid gap-1.5 text-sm font-medium" htmlFor="pending-period">Inatividade<select id="pending-period" aria-label="PerÃ­odo" className="flex h-10 rounded-md border border-input bg-background px-3 py-2 text-sm" value={periodDays ?? ''} onChange={(event) => update('periodDays', event.target.value)}><option value="">PadrÃ£o: 14 dias</option><option value="7">7 dias</option><option value="30">30 dias</option></select></label>
          {courses.isError && <p role="alert" className="text-sm text-destructive md:col-span-2">NÃ£o foi possÃ­vel carregar os cursos desta conexÃ£o.</p>}
        </CardContent>
      </Card>

      {!courseId && <Card className="border-dashed"><CardContent className="flex flex-col items-center gap-3 py-12 text-center"><AlertTriangle className="h-8 w-8 text-muted-foreground" /><div><h2 className="font-semibold">Selecione um curso</h2><p className="mt-1 text-sm text-muted-foreground">O App nÃ£o executa consulta agregada entre conexÃµes ou cursos.</p></div></CardContent></Card>}
      {courseId && query.isPending && <div className="grid gap-4 md:grid-cols-2"><Skeleton className="h-40" /><Skeleton className="h-40" /></div>}
      {courseId && query.isError && <Card><CardContent className="py-8"><p role="alert" className="text-destructive">NÃ£o foi possÃ­vel carregar as pendÃªncias.</p></CardContent></Card>}
      {courseId && query.isSuccess && query.data.data.length === 0 && <Card className="border-dashed"><CardContent className="flex flex-col items-center gap-3 py-12 text-center"><Clock3 className="h-8 w-8 text-muted-foreground" /><p className="text-sm text-muted-foreground">Nenhuma pendÃªncia encontrada neste curso.</p></CardContent></Card>}
      {courseId && query.isSuccess && query.data.data.length > 0 && <>
        <section className="grid gap-4 md:grid-cols-2" aria-label="Lista de pendÃªncias">
          {query.data.data.map((item) => <Card key={`${item.connectionRef}:${item.courseId}:${item.studentId}:${item.activityId ?? item.type}`} className="card-interactive"><CardHeader className="pb-3"><div className="flex items-start justify-between gap-3"><div><CardTitle className="text-base">{labels[item.type] ?? item.type}</CardTitle><CardDescription>{item.studentName} Â· {item.activityName}</CardDescription></div><Badge className={levelClasses[item.level]}>{levelLabels[item.level] ?? item.level}</Badge></div></CardHeader><CardContent className="space-y-4"><p className="text-sm text-muted-foreground">{item.factors.join(' ') || 'Sem fator adicional informado.'}</p>{item.type === 'awaiting_grading' && <p className="rounded-md bg-muted px-3 py-2 text-xs text-muted-foreground">Somente leitura: visualizar, filtrar ou abrir no Moodle. Nenhuma correÃ§Ã£o Ã© executada pelo App.</p>}<div className="flex flex-wrap items-center justify-between gap-2 border-t pt-3 text-xs text-muted-foreground"><span>{item.connectionRef} Â· {item.courseId}</span><Button variant="ghost" size="sm" asChild><Link to={`/alunos/${encodeURIComponent(item.connectionRef)}/${encodeURIComponent(item.courseId)}/${encodeURIComponent(item.studentId)}`}>Abrir aluno <ArrowRight className="h-4 w-4" /></Link></Button>{item.moodleUrl && <a className="text-primary hover:underline" href={item.moodleUrl} target="_blank" rel="noreferrer">Abrir no Moodle</a>}</div></CardContent></Card>)}
        </section>
