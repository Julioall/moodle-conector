  ...
  return (
    <div className="flex h-[calc(100vh-6rem)] flex-col gap-4 animate-fade-in">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-0 overflow-hidden rounded-2xl border lg:grid-cols-[360px_minmax(0,1fr)]">
        <div className="flex min-h-0 min-w-0 flex-col overflow-hidden border-r bg-card"><div className="shrink-0 border-b p-3"><div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input type="search" placeholder="Buscar conversa…" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} className="pl-9" aria-label="Buscar conversa" /></div><div className="mt-3 flex items-center justify-between gap-3 text-xs text-muted-foreground"><span>{conversations.length} {conversations.length === 1 ? 'conversa' : 'conversas'}</span>{conversationsQuery.isFetching && <span className="inline-flex items-center gap-2"><Spinner className="h-3.5 w-3.5" />Atualizando…</span>}</div></div><div className="min-h-0 flex-1 overflow-y-auto p-3">{conversationsQuery.isPending ? <div className="flex items-center justify-center py-12"><Spinner className="h-6 w-6" /></div> : filteredConversations.length === 0 ? <div className="flex flex-col items-center justify-center px-4 py-12 text-center"><MessageSquare className="mb-3 h-10 w-10 text-muted-foreground/50" /><p className="text-sm text-muted-foreground">{normalizedSearch ? 'Nenhuma conversa encontrada' : 'Nenhuma conversa no Moodle'}</p></div> : <div className="space-y-2">{filteredConversations.map((conversation) => <ConversationItem key={conversation.id} conversation={conversation} selected={selectedConversation?.id === conversation.id} onSelect={() => setSelectedConversationId(conversation.id)} />)}</div>}</div></div>
        ...
        <DialogContent>
          ...
          {pendingPreview && <div className="grid gap-4">
            <div className="rounded-xl border bg-muted/30 p-4 text-sm">
              ...
              <p className="mt-3 whitespace-pre-wrap break-words leading-6">{pendingPreview.messageText}</p>
            </div>
