...
export const createStudentsGateway = (client = createAppClient()) => ({
  byCourse: (connectionRef: string, courseId: string, page = 1, pageSize = 20, includePending = false) => client.get<StudentList>(`/api/courses/${encodeURIComponent(connectionRef)}/${encodeURIComponent(courseId)}/students?page=${page}&pageSize=${pageSize}${includePending ? '&includePending=true' : ''}`),
