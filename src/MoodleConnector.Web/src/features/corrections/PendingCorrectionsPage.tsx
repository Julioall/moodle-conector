...
export function PendingCorrectionsPage() {
  ...
  const courses = useQuery({ queryKey: ['app', 'corrections', 'courses', connectionRef], queryFn: () => coursesGateway.list(connectionRef, 1, 100), enabled: Boolean(connectionRef), staleTime: 60_000 });
  const activities = useQuery({ queryKey: ['app', 'corrections', 'activities', connectionRef, courseId], queryFn: () => coursesGateway.activities(connectionRef!, courseId, 1, 100), enabled: Boolean(connectionRef && courseId), staleTime: 60_000 });
  const pending = useQuery({ queryKey: ['app', 'pending', connectionRef, courseId], queryFn: () => pendingGateway.list(connectionRef!, courseId), enabled: Boolean(connectionRef && courseId), staleTime: 20_000 });
  const evidence = useQuery({ queryKey: ['app', 'evidence', connectionRef, courseId], queryFn: () => evidenceGateway.list(connectionRef!, courseId), enabled: Boolean(connectionRef && courseId), staleTime: 20_000 });
  const submissions = useQuery({ queryKey: ['app', 'submissions', connectionRef, courseId, assignmentId], queryFn: () => submissionsGateway.list(connectionRef!, courseId, assignmentId), enabled: Boolean(connectionRef && courseId && assignmentId), staleTime: 20_000 });
  const submissionDetail = useQuery({ queryKey: ['app', 'submission-detail', connectionRef, courseId, assignmentId, selectedSubmission?.userId], queryFn: () => submissionsGateway.detail(connectionRef!, courseId, assignmentId, selectedSubmission!.userId), enabled: Boolean(gradeOpen && connectionRef && courseId && assignmentId && selectedSubmission), staleTime: 20_000 });
  ...
  const submitPrepare = useMutation({
    mutationFn: () => submissionsGateway.prepareGrade({ connectionRef: connectionRef!, courseId, assignmentId, studentId: selectedSubmission!.userId, proposedGrade: Number(grade), feedbackText: feedback.trim() || undefined, justificationText: justification.trim() }),
    onSuccess: (response) => { setPrepareResult(response.data); setConfirmation(''); },
  });
  const confirmGrade = useMutation({
    mutationFn: () => submissionsGateway.confirmGrade({ connectionRef: connectionRef!, pendingActionId: prepareResult!.pendingActionId, confirmationText: confirmation }),
    onSuccess: () => { setGradeOpen(false); setPrepareResult(null); setSelectedSubmission(null); setGrade(''); setFeedback(''); setJustification(''); setConfirmation(''); void client.invalidateQueries({ queryKey: ['app', 'submissions', connectionRef, courseId, assignmentId] }); void client.invalidateQueries({ queryKey: ['app', 'pending', connectionRef, courseId] }); },
  });
