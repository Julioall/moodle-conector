...
export function PendingCorrectionsPage({ courseContext, embedded = false }: { courseContext?: CourseContext; embedded?: boolean }) {
  ...
  const activities = useQuery({ queryKey: ['app', 'corrections', 'activities', effectiveConnectionRef, courseId], queryFn: () => coursesGateway.activities(effectiveConnectionRef!, courseId, 1, 100), enabled: Boolean(effectiveConnectionRef && courseId), staleTime: 60_000, refetchInterval: (query) => query.state.data?.meta.source === 'background' || query.state.data?.meta.refreshQueued ? 2_000 : false });
  const pending = useQuery({ queryKey: ['app', 'pending', effectiveConnectionRef, courseId], queryFn: () => pendingGateway.list(effectiveConnectionRef!, courseId), enabled: Boolean(effectiveConnectionRef && courseId), staleTime: 20_000, refetchInterval: (query) => query.state.data?.meta.source === 'background' || query.state.data?.meta.refreshQueued || query.state.data?.meta.complete === false ? 2_000 : false });
  ...
  function refreshCorrections() {
    ...
    void client.fetchQuery({
      ...
      queryFn: () => coursesGateway.activities(effectiveConnectionRef, courseId, 1, 100, false, true),
      staleTime: 0,
    });
    void client.fetchQuery({
