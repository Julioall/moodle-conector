  ...
  return (
    <details className={`${level === 0 ? 'rounded-lg border bg-card' : 'border-l pl-4'} group`} open={open} onToggle={(event) => setOpen(event.currentTarget.open)}>
      ...
      <div className="space-y-3 px-4 pb-4">
        {[...node.children.values()].sort((left, right) => left.name.localeCompare(right.name, 'pt-BR')).map((child) => <TreeBranch key={child.path} node={child} connectionRef={connectionRef} statusFilter={statusFilter} editMode={editMode} ignoredCourseIds={ignoredCourseIds} onRestore={onRestore} level={level + 1} />)}
        ...
        {!hasChildren && filteredCourses.length > 0 && <div className="grid gap-4 pt-1 md:grid-cols-2 xl:grid-cols-3">{filteredCourses.map((course) => <CourseCard key={`${course.connectionRef}:${course.courseId}`} course={course} action={editMode && ignoredCourseIds.has(course.courseId) ? { label: 'Adicionar aos Meus Cursos', ariaLabel: `Adicionar ${course.displayName ?? course.fullName} aos Meus Cursos`, icon: <Plus className="h-4 w-4" />, onClick: () => onRestore(course.courseId) } : undefined} />)}</div>}
      </div>
    </details>
  );
}
...
export function SchoolsPage() {
  ...
  return (
    <main className="space-y-6 animate-fade-in" aria-labelledby="schools-title">
      ...
      {query.isSuccess && tree.children.size > 0 && <div className="space-y-3">{[...tree.children.values()].map((node) => <TreeBranch key={node.path} node={node} connectionRef={connectionRef} statusFilter={statusFilter} editMode={editMode} ignoredCourseIds={ignoredCourseIds} onRestore={restoreCourse} />)}</div>}
    </main>
  );
}
