...
describe('TopBar', () => {
  it('loads real Moodle connections and exposes a scoped selector', async () => {
    vi.mocked(connectionsGateway.list).mockResolvedValue({
      data: [
        { connectionRef: 'goias', alias: 'SENAI GoiÃ¡s', host: 'https://goias.example', status: 'unknown', isDefault: true, capabilities: ['read'] },
    ...
    expect((await screen.findAllByText('SENAI GoiÃ¡s')).length).toBeGreaterThan(0);
