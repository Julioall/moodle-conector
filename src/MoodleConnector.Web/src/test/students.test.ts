...
describe('Students gateway', () => {
  it('lists students with pagination and a composed Moodle identity', async () => {
    ...
    await gateway.list('senai-goias', 2);

    expect(client.get).toHaveBeenCalledWith('/api/portal/students?connectionRef=senai-goias&page=2&pageSize=20');
  });
  ...
  it('loads a read-only student profile using connectionRef and studentId', async () => {
    ...
    const response = await gateway.get('senai/goias', 'student/42');

    expect(client.get).toHaveBeenCalledWith('/api/portal/students/senai%2Fgoias/student%2F42');
