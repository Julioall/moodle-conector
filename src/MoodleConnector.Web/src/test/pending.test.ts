...
describe('pending gateway', () => {
  it('encodes deterministic filters and keeps the read-only endpoint', async () => {
    ...
    await createPendingGateway(client).list({ connectionRef: 'senai goias', courseId: '42', studentId: '7', type: 'awaiting_grading', level: 'risk', periodDays: 7, page: 2 });
    expect(client.get).toHaveBeenCalledWith(expect.stringContaining('/api/pending?'));
    ...
    expect(client.get.mock.calls[0][0]).toContain('periodDays=7');
