...
> **Nota:** A função `mod_forum_get_forum_discussions_paginated` foi deprecada e removida no Moodle 5.0. Nas conexões FIEG e SENAI, o conector consulta as funções anunciadas por `core_webservice_get_site_info` e usa `mod_forum_get_forum_discussions`; só recorre à variante paginada quando ela for a única disponível na conexão.
...
Além das permissões Moodle da função, a conexão cadastrada precisa estar com `CanWrite=true`, o chamador precisa ter o escopo aplicável e a feature flag do domínio precisa estar ativa. A escrita universal está habilitada no perfil versionado e no deploy padrão, mas Production exige manifesto de contratos verificados; cada ambiente ainda pode desligá-la explicitamente. As flags bloqueiam preparação/confirmação quando definidas como `false`.
