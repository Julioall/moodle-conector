...
|---|---|---|
| `MoodleCoursesTools` | cursos; R; não; não uniforme; testes de cursos/capacidades | `listar_meus_cursos`, `list_courses`, `buscar_cursos`, `search_courses`, `search`, `consultar_curso`, `get_course`, `fetch` |
...
| `MoodleParticipantsTools` | participantes; R; não; não uniforme; testes de categoria | `listar_participantes_curso`, `list_course_participants`, `listar_alunos_curso`, `list_course_students`, `listar_grupos_curso`, `list_course_groups`, `consultar_membros_grupo`, `get_group_members` |
| `MoodleCourseContentsTools` | conteúdos; R; não; não uniforme; testes de categoria | `listar_conteudos_curso`, `list_course_contents`, `consultar_modulo_curso`, `get_course_module`, `listar_recursos_curso`, `list_course_resources`, `listar_arquivos_curso`, `list_course_files`, `listar_paginas_curso`, `list_course_pages`, `listar_urls_curso`, `list_course_urls`, `auditar_estrutura_curso`, `audit_course_structure` |
| `MoodleCourseActivitiesTools` | atividades; R; não; não uniforme; testes de categoria | `listar_atividades_curso`, `list_course_activities`, `consultar_atividade`, `get_course_activity`, `listar_tarefas_curso`, `list_course_assignments`, `consultar_tarefa`, `get_assignment`, `listar_quizzes_curso`, `list_course_quizzes`, `consultar_quiz`, `get_quiz`, `listar_scorms_curso`, `list_course_scorms`, `consultar_prazos_atividades`, `list_activity_deadlines` |
| `MoodleForumTools` | fórum; R/W; sim para publicação; pendente/confirmado; testes específicos | `ler_forum`, `read_forum`, `criar_previa_post_forum`, `create_forum_post_preview`, `confirmar_post_forum_moodle`, `confirm_forum_post` |
| `MoodleAssignmentSubmissionsTools` | entregas; R; não; não uniforme; testes de categoria | `listar_entregas_atividade`, `list_assignment_submissions`, `consultar_entrega_aluno`, `get_student_submission`, `listar_entregas_pendentes`, `list_pending_submissions`, `listar_entregas_atrasadas`, `list_late_submissions`, `listar_entregas_aguardando_correcao`, `list_submissions_awaiting_grading`, `consultar_status_submissao`, `get_submission_status` |
| `MoodleGradingTools` | correção; R/W; parcial; lote/auditoria própria; testes de correção | `descobrir_funcoes_moodle_correcao`, `discover_moodle_grading_functions`, `executar_descoberta_tecnica_correcao`, `listar_entregas_corrigiveis`, `criar_lote_correcao_assistida`, `consultar_status_lote_correcao`, `exportar_relatorio_correcao_coordenacao`, `cancelar_lote_correcao_assistida`, `consultar_item_correcao_assistida`, `atualizar_rascunho_correcao`, `atualizar_rascunhos_correcao_lote`, `criar_previa_lancamento_lote`, `confirmar_lancamento_lote_moodle`, `consultar_auditoria_correcao`, `consultar_auditoria_correcao_lote`, `preparar_lote_correcao_ia`, `salvar_correcoes_ia_lote`, `preparar_correcao_entrega` |
| `MoodleIndividualGradeTools` | nota individual; W; sim; parcial; testes de comandos, sem concorrência de banco | `preparar_lancamento_nota`, `prepare_individual_grade_launch`, `confirmar_lancamento_nota`, `confirm_individual_grade_launch` |
| `MoodleGradebookTools` | boletim; R; não; não uniforme; testes de categoria | `consultar_boletim_aluno`, `get_student_gradebook` |
| `MoodleCompletionTools` | progresso; R; não; não uniforme; testes de categoria | `consultar_progresso_aluno`, `get_student_completion` |
| `MoodleRiskAnalysisTools` | risco; R; não; não uniforme; testes de categoria | `gerar_relatorio_risco_estudantes`, `report_students_at_risk` |
| `MoodleGradingContextDiagnosticsTools` | diagnóstico de correção; R; não; consulta de auditoria; testes de correção | `consultar_contexto_item_correcao_assistida` |
| `MoodleGradingReviewAppTools` | revisão; R; não; consulta de lote; testes de correção | `revisar_feedbacks_lote` |
| `MoodleTutorMessageTools` | mensagens; preparar R, confirmar W; sim; pending/auditoria; testes de mensagens | `preparar_mensagem_boas_vindas`, `confirmar_mensagem_boas_vindas`, `preparar_mensagem_cobranca_acesso`, `confirmar_mensagem_cobranca_acesso`, `preparar_mensagem_cobranca_sa`, `confirmar_mensagem_cobranca_sa`, `preparar_mensagem_recuperacao`, `confirmar_mensagem_recuperacao`, `preparar_mensagem_encerramento`, `confirmar_mensagem_encerramento`, `preparar_mensagem_acompanhamento`, `confirmar_mensagem_acompanhamento` |
| `MoodleReportTools` | relatórios; R; não; não uniforme; testes de categoria | `gerar_relatorio_semanal_desempenho`, `generate_weekly_performance_report`, `gerar_relatorio_turma_conselho_classe`, `generate_class_council_report`, `gerar_resumo_curso`, `gerar_relatorio_pos_execucao_completo` |
| `MoodleMonitorTools` | monitoria; R; não; não uniforme; testes de categoria | `auditar_checklist_sala_virtual`, `audit_virtual_classroom_checklist`, `gerar_relatorio_monitor_turma`, `generate_monitor_class_report` |
| `MoodleMemoryTools` | memória local; W; não; não se aplica a Moodle; testes de memória | `gerenciar_memoria_usuario` |
| `MoodleMemoryDocumentTools` | documentos de memória local; R/W; não; não se aplica a Moodle; testes de memória | `gerenciar_documento_memoria_usuario`, `salvar_documento_memoria_usuario`, `listar_documentos_memoria_usuario`, `ler_documento_memoria_usuario`, `remover_documento_memoria_usuario` |
| `MoodlePedagogyTools` | orientação pedagógica; R; não; não se aplica diretamente a Moodle; testes de categoria | `consultar_orientacoes_pedagogicas` |

**Implementadas, mas não registradas:** `MoodleForumParticipationTools` (`listar_alunos_sem_participacao_forum`, `list_students_without_forum_participation`), `MoodleAccessMonitoringTools` (`listar_alunos_sem_acesso`, `list_students_without_recent_access`), `MoodleStudentPerformanceTools` (`consultar_desempenho_estudante_por_atividade`, `get_student_activity_grades`, `listar_alunos_abaixo_minimo`, `list_students_below_min_grade`) e `MoodlePendingSubmissionsTools` (`listar_alunos_pendentes_atividade`, `list_students_with_pending_submissions`).

**Demonstração não ativa:** `DemoPendingActionTools` (`preparar_acao_demo`, `confirmar_acao_demo`) só é registrada se `DemoToolsEnabled=true`, hoje `false`.
